...
The manifest is intentionally generated by elevata and should not be edited manually.

It contains:

- all target datasets  
- lineage dependencies  
- lineage dependencies between datasets  
...
2. The task `generate_manifest` runs `elevata_manifest`.  
3. The DAG executes dataset loads according to the previously generated manifest.  
...
| `ELEVATA_CMD` | command used to invoke elevata (default: `python manage.py`) |
...
By default, the Airflow example image installs:

- `requirements/base.txt`  
- `requirements/duckdb.txt`

To build the image with a different backend requirements file, override the build arg:

```bash
cd examples/airflow
docker compose build --build-arg ELEVATA_REQ_BACKEND=requirements/databricks.txt
docker compose up
```

Build arg precedence:

...
elevata introduces a **Meta-SQL Layer**, which is now internally realized through the Logical Plan and the SQL Preview pipeline that captures *what* each dataset represents, *how* it is derived, and *which relationships and keys* define it.  
...
Future releases will extend this compiler with full materialization and orchestration interfaces.

Ultimately, elevata’s goal is not to replace dbt - but to **make dbt optional** by moving the intelligence where it belongs: into metadata-driven, declarative architecture.
